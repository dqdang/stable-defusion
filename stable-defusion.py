...
def xgb_model(X_train, y_train, X_test, y_test):
    ...
    keras_model.set_weights(model.get_dump(with_stats=True))
    keras_model.compile(optimizer=model.optimizer, loss=model.loss, metrics=model.metrics)

    # pickle.dump(model, open("cnn_xgboost_final.pickle.dat", "wb"))
    keras_model.save('stable-defusion.h5')

    converter = lite.TFLiteConverter.from_keras_model_file('stable-defusion.h5')
    tfmodel = converter.convert()
    open("stable-defusion.tflite" , "wb").write(tfmodel)
...
def main():
    ...
    print("Build and save the CNN-XGBoost Model.")
